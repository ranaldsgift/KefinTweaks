/**
 * KefinTweaks Multi-User Sessions / Watch Together
...
(function () {
    ...
    const LOG = (...args) => console.log('[KefinTweaks MultiUserSession]', ...args);
    const WARN = (...args) => console.warn('[KefinTweaks MultiUserSession]', ...args);
    const ERR = (...args) => console.error('[KefinTweaks MultiUserSession]', ...args);

    const DEVICE_STORAGE_PREFIX = 'kefinTweaks_watchTogether';
    const FEATURE_KEY = 'watchTogether';
    const MODAL_ID = 'kefin-multi-user-session-modal';
    const ADD_USER_MODAL_ID = 'kefin-multi-user-session-add-modal';
    ...
    function injectStyles() {
        if (document.getElementById('kefin-multi-user-session-styles')) return;
        const style = document.createElement('style');
        style.id = 'kefin-multi-user-session-styles';
        style.textContent = `
            .headerUserButton[data-multi-user] .material-icons.person::before,
            [data-multi-user].material-icons.person::before {
                content: "\\e7ef";
            }
            ...
            .mus-watch-together-avatar-wrap:focus-within .mus-avatar-remove {
                opacity: 1;
                pointer-events: auto;
            }
    ...
    function writeDeviceState(state) {
        ...
        try {
            ...
            updateHeaderMultiUserIndicator();
            updateSyncListenerState();
            try {
                refreshPreferencesMenuLinks();
            } catch (uiErr) {
                WARN('Failed to refresh preferences menu links:', uiErr);
            }
            return true;
        } catch (e) {
    ...
    function getSessionTimeoutMs() {
        const minutes = window.KefinTweaksConfig?.multiUserSession?.sessionTimeout;
    ...
    function updateHeaderMultiUserIndicator() {
        ...
        document.querySelectorAll('.headerUserButton').forEach((btn) => {
            if (hasMulti) {
                if (!btn.hasAttribute('data-mus-original-title')) {
                    const existing = btn.getAttribute('title');
                    if (existing) btn.setAttribute('data-mus-original-title', existing);
                }
                if (multiTitle) btn.setAttribute('title', multiTitle);
                btn.setAttribute('data-multi-user', 'true');
                btn.querySelectorAll('.material-icons.person').forEach((icon) => {
                    icon.setAttribute('data-multi-user', 'true');
                });
            } else {
                if (btn.hasAttribute('data-mus-original-title')) {
                    const original = btn.getAttribute('data-mus-original-title');
                    if (original) btn.setAttribute('title', original);
                    else btn.removeAttribute('title');
                    btn.removeAttribute('data-mus-original-title');
                }
                btn.removeAttribute('data-multi-user');
                btn.querySelectorAll('[data-multi-user]').forEach((el) => {
                    el.removeAttribute('data-multi-user');
                });
            }
        });
    }
    ...
    async function syncUserDataToSessionUsers(userData) {
        ...
        try {
            ...
            syncMirrorDepth = Math.max(0, syncMirrorDepth - 1);
        }
    }
        ...
        root.querySelectorAll('.mus-watch-together-avatar').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                if (applyImmediately) {
                    ...
                    updateWatchStateSyncListIcon(
                        document.querySelector('.userPreferencesPage:not(.hide) [data-kefin-watch-state-sync]')
                    );
                }
            });
        });
    ...
    function openWatchTogetherPrompt(options = {}) {
        const {
            ...
            hint = 'Watch state will sync to the active accounts in your group as you watch new content.'
        } = options;
        ...
        watchTogetherModal = window.ModalSystem.create({
            ...
            footer: showAddUser
                ? `
                <div class="mus-wt-footer-actions" style="display:flex;flex-direction:row;flex-wrap:wrap;gap:0.5em;width:100%;justify-content:flex-end;align-items:center;">
                    <button type="button" class="raised button-submit emby-button" id="mus-wt-add-user">Add User</button>
                </div>
            `
                : null,
            closeOnBackdrop: true,
    ...
    function getVisiblePreferencesPage() {
        return document.querySelector('.userPreferencesPage:not(.hide)');
    }

    function getVisiblePreferencesUserSection(preferencesPage = null) {
        const page = preferencesPage || getVisiblePreferencesPage();
        if (!page) return null;
        return page.querySelector('.userSection.verticalSection');
    }

    function getWatchTogetherMenuIcon() {
        return 'group';
    }

    function updateWatchStateSyncListIcon(link) {
        const iconEl = link?.querySelector('.listItemIcon');
        if (!iconEl) return;
        iconEl.className = `material-icons listItemIcon listItemIcon-transparent ${getWatchTogetherMenuIcon()}`;
    }

    function createPreferencesListLink({ markerAttr, icon, title, titleHtml, subtitle, subtitleHtml, onClick }) {
        ...
        link.setAttribute(markerAttr, 'true');
        const titleContent = titleHtml
            ? `<div class="listItemBodyText mus-watch-sync-title">${titleHtml}</div>`
            : `<div class="listItemBodyText">${escapeHtml(title)}</div>`;
        const subtitleContent = subtitleHtml
            ? `<div class="listItemBodyText secondary mus-watch-sync-names">${subtitleHtml}</div>`
            : (subtitle ? `<div class="listItemBodyText secondary">${escapeHtml(subtitle)}</div>` : '');
        link.innerHTML = `
            <div class="listItem">
                <span class="material-icons listItemIcon listItemIcon-transparent ${icon}" aria-hidden="true"></span>
    ...
    function refreshPreferencesMenuLinks() {
        const preferencesPage = getVisiblePreferencesPage();
        if (!preferencesPage) return;

        const userSection = getVisiblePreferencesUserSection(preferencesPage);
        if (!userSection) return;

        // Legacy prefs entry removed from the menu; clean up if still present.
        preferencesPage.querySelector('[data-kefin-add-user-session]')?.remove();

        let syncLink = preferencesPage.querySelector('[data-kefin-watch-state-sync]');
        if (!syncLink) {
            syncLink = createPreferencesListLink({
                markerAttr: 'data-kefin-watch-state-sync',
                icon: getWatchTogetherMenuIcon(),
                title: 'Watch Together',
                onClick: openManageModal
            });
            const signOut = userSection.querySelector('.btnLogout');
            if (signOut && signOut.parentNode === userSection) {
                userSection.insertBefore(syncLink, signOut);
            } else {
                userSection.appendChild(syncLink);
            }
            return;
        }

        updateWatchStateSyncListIcon(syncLink);
    }

    function injectPreferencesMenuLinks() {
        const preferencesPage = getVisiblePreferencesPage();
        if (!preferencesPage) return;
        ...
        refreshPreferencesMenuLinks();
        LOG('Preferences menu links injected');
    }
    ...
    function registerPreferencesPageHandler() {
        ...
        window.KefinTweaksUtils.onViewPage((view, element, hash) => {
            if (!hash || !(
                ...
                || hash.includes('userpreferences')
                || hash.includes('preferences')
            )) {
                return;
            }
            setTimeout(injectPreferencesMenuLinks, 300);
            setTimeout(injectPreferencesMenuLinks, 800);
        }, {
            pages: ['mypreferencesmenu']
        });

        LOG('Preferences onViewPage handler registered');
    }
